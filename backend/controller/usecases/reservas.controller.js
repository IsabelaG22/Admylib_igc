...
exports.guardarReserva = async (req, res) => {
  ...
  exports.buscarReservas = async (req, res) => {
    try {
      ...
      if (busquedaReservas.respuesta === false) {
        ...
        res.status(200).json({ Reserva: 'Reservas encontradas' + busquedaReservas });
      }
    } catch (err) {
      res.status(500).json({ error: err });
    }
  ...
  exports.buscarReservaById = async (id)=>{
    try {
        ...
        if (busquedaReservaById.respuesta === false) {
          ...
          res.status(200).json({ Reserva: 'Reserva encontrada' + busquedaReservaById });
        }
      } catch (err) {
        res.status(500).json({ error: err });
      }
